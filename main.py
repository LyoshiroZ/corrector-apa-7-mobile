...
# Si la app crashea al iniciar, escribimos el traceback completo en el telefono
# en /sdcard/Download/apa_crash.log para poder diagnosticar.
def _ruta_log_crash() -> str:
    try:
        from kivy.utils import platform as _plt
        if _plt == "android":
            from android.storage import primary_external_storage_path
            return os.path.join(
                primary_external_storage_path(), "Download", "apa_crash.log"
            )
    except Exception:
        pass
    return os.path.join(os.path.expanduser("~"), "apa_crash.log")


def _escribir_crash(exc_type, exc_value, exc_tb):
    try:
        with open(_ruta_log_crash(), "w", encoding="utf-8") as f:
            f.write("=== APA Crash Log ===\n")
