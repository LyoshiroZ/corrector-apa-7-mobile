...
class CorrectorApp(MDApp):
    ...
    def _descargar_plantilla(self, nombre: str):
        ...
        try:
            ...
            if visible:
                self._snack(f"Plantilla '{nombre}' guardada en {visible}")
                return
            ...
            with open(ruta, "wb") as f:
                f.write(data)
            self._snack(f"Plantilla '{nombre}' guardada: "
    ...
    def _descargar_word(self):
        try:
            ...
            # Intento 1: Descargas pública via MediaStore (Android 10+)
            visible = guardar_en_downloads_publicos(nombre, data, mime)
            if visible:
                self._snack(f"Guardado en {visible}")
                return
            # Fallback: carpeta privada de la app (todas las versiones)
            destino = directorio_descargas()
            ruta = os.path.join(destino, nombre)
            i = 1
            while os.path.exists(ruta):
                ruta = os.path.join(
                    destino, f"{nombre_base}_{i}.docx"
                )
                i += 1
            with open(ruta, "wb") as f:
                f.write(data)
