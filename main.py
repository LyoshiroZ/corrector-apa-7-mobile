...
class CorrectorApp(MDApp):
    ...
    def _build_home(self) -> HomeScreen:
        ...
        # ---- Cómo usar APU-APA (3 pasos como en la web) ----
        body.add_widget(self._guia_uso())
        ...
        # ---- Card "Ejemplos APA" (info útil como en la web) ----
        body.add_widget(self._tarjeta_accion(
            icono="book-open-variant",
            titulo="Ejemplos de citas APA",
            subtitulo="Ver ejemplos rápidos de citas y referencias",
            color_borde=ACENTO,
            on_press=self._mostrar_ejemplos,
        ))

        # ---- Card "Plantillas APA" (descarga Word formateado) ----
        body.add_widget(self._tarjeta_accion(
            icono="file-document-multiple",
            titulo="Plantillas APA",
            subtitulo="Descarga un Word ya formateado: ensayo, tesis, etc.",
            color_borde=PRIMARIO,
            on_press=self._abrir_menu_plantillas,
        ))
        ...
        # Contador de palabras (igual que la web "0 palabras")
        ...
        body.add_widget(self.contador_label)
    ...
    def _tarjeta_accion(self, icono, titulo, subtitulo, on_press,
        ...
        card.add_widget(MDIconButton(
            ...
            text_color=color_borde, icon_size=dp(36), disabled=True,
        ))
        col = MDBoxLayout(orientation="vertical", spacing=dp(2))
        col.add_widget(MDLabel(
    ...
    # ---- Nuevas secciones estilo web --------------------------------------

    def _guia_uso(self) -> MDCard:
        """Card expandible '¿Cómo usar APU-APA?' con 3 pasos como en la web."""
        card = MDCard(
            orientation="vertical",
            ...
            md_bg_color=(1, 1, 1, 1),
            line_color=CREMA_BORDE,
            radius=[dp(12)] * 4, elevation=0,
            adaptive_height=True,
        )
        card.add_widget(MDLabel(
            text="📖  ¿Cómo usar APU-APA? (guía rápida)",
            theme_text_color="Custom", text_color=PRIMARIO,
            font_style="Subtitle1", bold=True,
            size_hint_y=None, height=dp(28),
        ))
        pasos = [
            ("1️⃣  Ingresa tu texto",
             "Pega tu ensayo o sube un archivo Word (.docx).",
             PRIMARIO),
            ("2️⃣  Configura opciones",
             "Pulsa el engranaje arriba para tu API key, modelo Gemini "
             "y tipo de documento.",
             ACENTO),
            ("3️⃣  Corrige y descarga",
             "Pulsa 'Corregir formato APA 7' y descarga el Word ya "
             "formateado.",
             PRIMARIO),
        ]
        for titulo, sub, color in pasos:
            paso = MDBoxLayout(
                orientation="vertical",
            ...
            # Hack para que MDBoxLayout tenga md_bg_color: usar MDCard interno
            sub_card = MDCard(
                orientation="vertical",
                padding=(dp(10), dp(8), dp(10), dp(8)),
                spacing=dp(2),
            ...
            card.add_widget(sub_card)
        return card

    def _mostrar_ejemplos(self):
        """Diálogo con ejemplos APA 7 (libro, artículo, web)."""
        ejemplos = (
            "[b][color=2D5016]📚 Libro[/color][/b]\n"
            "[size=12sp]Apellido, A. (Año). [i]Título del libro[/i] "
            "(Edición). Editorial.[/size]\n\n"
            "[b][color=8B4513]📄 Artículo de revista[/color][/b]\n"
            "[size=12sp]Apellido, A. (Año). Título del artículo. "
            "[i]Revista[/i], vol(núm), pp-pp. https://doi.org/...[/size]\n\n"
            "[b][color=2D5016]🌐 Página web[/color][/b]\n"
            "[size=12sp]Apellido, A. (Año, día de mes). Título. "
            "[i]Sitio web[/i]. URL[/size]\n\n"
            "[b][color=8B4513]✏️ Cita en el texto[/color][/b]\n"
            "[size=12sp]Paréntesis: (Apellido, año, p. X)\n"
            "Narrativa: Apellido (año) afirma que...[/size]"
        )
        contenido = MDLabel(
            text=ejemplos, markup=True,
            theme_text_color="Custom", text_color=TEXTO_OSCURO,
            size_hint_y=None,
        )
        contenido.bind(texture_size=lambda i, v: setattr(i, "height", v[1]))
        scroll = MDScrollView(size_hint=(1, None), height=dp(380))
        scroll.add_widget(contenido)
        dlg = MDDialog(
            title="Ejemplos APA 7ma",
            type="custom",
            content_cls=scroll,
            buttons=[
                MDFlatButton(
    ...
    def _abrir_menu_plantillas(self):
        """Diálogo con la lista de plantillas APA disponibles."""
        ...
        for nombre, datos in PLANTILLAS_APA.items():
            ...
            # Botón principal — ahora SÍ dispara on_release confiable
            btn = MDRaisedButton(
                text=f"  📄   {nombre}",
                md_bg_color=PRIMARIO,
                text_color=(1, 1, 1, 1),
        ...
        scroll = MDScrollView(size_hint=(1, None), height=dp(460))
        scroll.add_widget(contenido)
        self._dlg_plantillas = MDDialog(
            title="📦 Plantillas APA 7",
            type="custom",
            content_cls=scroll,
            buttons=[
                MDFlatButton(
                    text="Cerrar",
                    theme_text_color="Custom", text_color=PRIMARIO,
                    on_release=lambda *_: self._dlg_plantillas.dismiss(),
                ),
            ],
        )
        self._dlg_plantillas.open()

    def _descargar_plantilla(self, nombre: str):
        """Genera la plantilla y la guarda en Descargas."""
        # Cerramos diálogo DESPUÉS de programar la descarga (evita carrera)
        try:
            data = construir_plantilla_apa(nombre)
            destino = directorio_descargas()
            os.makedirs(destino, exist_ok=True)
            slug = re.sub(r"[^\w\-]+", "_", nombre.lower())
            ruta = os.path.join(destino, f"plantilla_apa_{slug}.docx")
            i = 1
            while os.path.exists(ruta):
                ruta = os.path.join(destino,
                                    f"plantilla_apa_{slug}_{i}.docx")
                i += 1
            with open(ruta, "wb") as f:
                f.write(data)
            if hasattr(self, "_dlg_plantillas"):
                try:
                    self._dlg_plantillas.dismiss()
                except Exception:
                    pass
            self._snack(f"📥 Plantilla '{nombre}' guardada en Descargas: "
